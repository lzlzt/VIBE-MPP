...
class GATConv(MessagePassing):
    def __init__(self,
        ...
        self.edge_update2 = Parameter(torch.Tensor(emb_dim + self.edge_dim, emb_dim))   # [emb(out)+edge_dim] x emb(out)  # new
    ...
    def update(self, aggr_out):   
        ...
        aggr_out = torch.mm(aggr_out, self.edge_update2)
...
class MPNNConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # Message-passing: Define the message function
        messages = self.message(x=edge_attr, edge_index=edge_index)

        # Aggregation: Aggregate messages using specified method
        aggr_out = self.aggregate(messages, edge_index, dim_size=x.size(0))

        # Update: Pass aggregated messages through an MLP
        updated_x = self.update(aggr_out)

        return updated_x

    def message(self, x_j, edge_index):
        # Message function: concatenate node features and edge attributes
        return torch.cat([x_j, edge_index[None, :].permute(1, 0, 2).float()], dim=-1)

    def update(self, aggr_out):
        # Update function: pass aggregated messages through an MLP
    ...
    def __init__(self, num_layer, emb_dim, JK = "last", drop_ratio = 0, gnn_type = "gin"):
        ...
        for layer in range(num_layer):
            if gnn_type == "gin":
                ...
                self.gnns.append(MPNNConv(emb_dim))
